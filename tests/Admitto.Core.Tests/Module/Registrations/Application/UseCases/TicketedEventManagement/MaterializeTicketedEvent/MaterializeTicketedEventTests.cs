...
using Amolenk.Admitto.Core.Registrations.Application.UseCases.TicketedEventManagement.MaterializeTicketedEvent;
...
public sealed class MaterializeTicketedEventTests(TestContext testContext) : AspireIntegrationTestBase
{
    ...
    [TestMethod]
    public async ValueTask SC001_Materialize_NewRequest_CreatesAggregateAndRaisesDomainEvent()
    {
        ...
        var sut = new TicketedEventCreationRequestedIntegrationEventHandler(
            new MaterializeTicketedEventHandler(Environment.Database.Context));
        ...
        var addedEntry = Environment.Database.Context.ChangeTracker
