...
3. **Verify that `now < event.StartsAt`**; return HTTP 409 with reason "event has already started" if the event has already started or is in progress.
...
- **GIVEN** a registration in state `Registered` with id "reg-abc" on event "devconf-2026" which starts in the future
...
- **GIVEN** a registration in state `Registered` with id "reg-abc" on event "devconf-2026" whose `StartsAt` is in the past
- **WHEN** the attendee posts to the cancel endpoint
