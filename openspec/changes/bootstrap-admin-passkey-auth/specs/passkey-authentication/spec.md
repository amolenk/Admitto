...
In production, the system SHALL use passkeys (WebAuthn passwordless credentials) as the only authentication factor for Admin UI users. Authentication SHALL be performed by Keycloak's hosted login and enrollment pages; the Admitto codebase SHALL NOT implement the WebAuthn ceremony itself. Passwords, one-time codes, and other non-passkey factors SHALL NOT be offered as primary authentication for production Admin UI users. All Keycloak pages reached by the supported production sign-in, passkey enrollment, required-action, error, info, and logout flows SHALL use the Admitto login theme.
...
#### Scenario: Production passkey assertion page is themed
- **WHEN** a production user reaches the Keycloak passkey assertion page
- **THEN** the page uses the Admitto login theme and does not fall back to generic Keycloak branding
...
#### Scenario: Production passkey enrollment page is themed
- **WHEN** a bootstrap administrator follows the enrollment action link and reaches the passkey registration page
- **THEN** the page uses the Admitto login theme and does not fall back to generic Keycloak branding

#### Scenario: Production required-action error page is themed
- **WHEN** a production user's required-action link is expired or invalid
- **THEN** the error or information page uses the Admitto login theme and presents a consistent Admitto-branded experience
