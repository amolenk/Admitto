...
- Ensure production Keycloak login, enrollment, required-action, error, info, and logout pages use Admitto branding and do not fall back to generic Keycloak screens.
...
### The Keycloak theme covers the full production auth journey

The production Keycloak image must include Admitto theme templates for every page a user can reach during the passkey-only journey. At minimum this includes login/identifier, WebAuthn passwordless assertion, WebAuthn passwordless registration, required-action dispatch, execute-actions token handling, informational success pages, error/expired-token pages, and logout confirmation. The implementation should prefer overriding only the required FreeMarker templates while continuing to inherit shared layout/assets from `keycloak.v2`.

The theme coverage must be validated against the production realm flow, not just by checking for a custom `login.ftl`. Any page in the supported production bootstrap/sign-in path that falls back to generic Keycloak branding is a gap.

Alternative considered: rely on Keycloak parent theme for non-login pages. This is acceptable for local development but not production because enrollment and error pages are part of the first-admin trust experience.
...
- [Risk] A production passkey page falls back to the generic Keycloak parent theme. → Add theme coverage checks for all pages in the configured browser and required-action flows.
