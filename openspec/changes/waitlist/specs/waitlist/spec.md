...
that is in WaitlistOnly mode. The attendee provides their email address. The system
SHALL send a verification email containing a short-lived HMAC-signed link; the entry
is only added to the waitlist once the attendee clicks that link. The attendee is
assigned the next position in the ranked queue. An attendee with the same email
address SHALL NOT be added twice for the same ticket type; a duplicate request SHALL
succeed silently (idempotent — no new verification email is sent).

#### Scenario: Successfully join the waitlist
- **WHEN** attendee "alice@example.com" submits a waitlist join request for ticket
  type "General Admission" on event "DevConf" which is in WaitlistOnly mode
- **THEN** a verification email is sent to "alice@example.com" containing a signed
  verification link valid for 24 hours, and no waitlist entry yet exists in the queue

#### Scenario: Email verified — entry added to waitlist
- **WHEN** "alice@example.com" clicks the signed verification link for ticket type
  "General Admission" on event "DevConf"
- **THEN** an active waitlist entry is created for "alice@example.com" at the next
  queue position
...
- **WHEN** "alice@example.com" is already on the waitlist for "General Admission" and
  submits another join request
- **THEN** the response is 200 OK with no new verification email sent and no duplicate
  entry created
...
When a waitlist coupon's claim window expires without redemption the system SHALL
revoke the coupon and trigger notification for the next batch of waitlist entries
(subject to the same quiet-hours rules).
...
#### Scenario: Coupon redeemed — claim window closes successfully
- **WHEN** "alice@example.com" redeems their waitlist coupon before `ExpiresAt`
- **THEN** a registration is created via the standard coupon registration flow; the
  coupon is marked redeemed; no further waitlist processing occurs for that slot
