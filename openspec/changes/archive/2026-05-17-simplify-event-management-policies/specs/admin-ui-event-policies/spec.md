...
**Reason**: `TicketedEventCancellationPolicy` is removed from the domain (see `event-management` delta). The Cancellation Policy page and its route are deleted.
**Migration**: Remove the `/settings/cancellation` route and page. Remove the "Cancellation" link from the sidebar Policies section.
...
- a cadence expressed in days, and
...
- close is after open,
- cadence is a positive integer ≥ 1, and
- minimum email interval is a positive integer ≥ 1.
...
- **WHEN** an organizer sets the reconfirm window to "2025-05-01T00:00Z" / "2025-05-25T00:00Z", cadence to 7 days, and minimum email interval to 48 hours, and submits
