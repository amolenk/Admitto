...
- **GIVEN** a registration with id "reg-abc" holding ["Early Bird"] on an active event with open registration, and "Workshop" has capacity 5/20 used
...
- **WHEN** the attendee submits a change request with a registration ID that does not exist, belongs to a different event, or belongs to a different team than the API key owner
...
- **WHEN** the attendee submits the same selection ["General Admission"] with a valid API key and without an Authorization header
...
- **WHEN** the attendee submits to the ticket-change endpoint without `X-Api-Key`
- **THEN** the system returns HTTP 401 and does not run the ticket-change handler
