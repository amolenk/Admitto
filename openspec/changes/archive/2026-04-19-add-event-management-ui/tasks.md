...
- [x] 9.2 Add CLI commands for: open/close registration, get open-status (Registrations). _Resolved by `restore-cli-parity` — see `Commands/Events/Registration/{Open,Close,ShowRegistrationStatus}Command.cs`._
- [x] 9.3 Add CLI commands for: get/update event email settings (Email). _Resolved by `restore-cli-parity` — see `Commands/Events/Email/{ShowEventEmail,UpdateEventEmail}Command.cs`._
- [x] 9.4 Regenerate `Admitto.Cli/Api/ApiClient.g.cs` via the `cli-api-client-generation` skill. _Resolved by `restore-cli-parity`: client regenerated against the live API; commands referencing removed admin types were quarantined via `<Compile Remove>` in the csproj rather than rewritten. See that change's `design.md` for the full quarantined surface._
