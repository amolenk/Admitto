...
When the admin path resets a cancelled registration, it SHALL preserve the existing `RegistrationId`, transition the status to `Registered`, set `CreatedAt` to the reset time, clear cancellation metadata, clear reconfirmation state, replace attendee details, replace ticket snapshots, replace additional details, and produce the same post-creation effects as the other successful registration paths. There is no "silent add" mode in this change.
...
- **THEN** the existing registration is reset to `Registered`, its original registration id is returned, its creation timestamp is the reset time, its attendee details, ticket snapshots, and additional details match the new request, its cancellation metadata and reconfirmation state are cleared, capacity is claimed with admin bypass rules, and attendee-registered side effects are produced
