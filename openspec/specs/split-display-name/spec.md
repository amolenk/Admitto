# Split Display Name Specification

## Purpose

Replace the generic `DisplayName` value object with domain-specific name VOs (`TeamName`, `EventName`, `TicketTypeName`) so that each domain entity carries a strongly-typed name that cannot be confused with another entity's name at compile time.
