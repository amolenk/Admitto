...
Team admins can directly add a registration to a ticketed event without going through the public self-service flow or requiring a coupon. The admin path bypasses the registration window, per-ticket-type capacity limits, the requirement that selected ticket types have an explicit capacity configured, and any configured email-domain restriction, while still enforcing event lifecycle status, ticket-type validity, duplicate-email guards, and `AdditionalDetailSchema` validation. The capability is exposed via an admin HTTP endpoint and a matching CLI command.
