...
The Email module SHALL expose a query that returns, for any `BulkEmailJob`, the full snapshot (recipients with per-recipient status, source, job-owned content if any, trigger user, timestamps, status, totals, last error). This is the data backing both the admin endpoint and the Admin UI detail page.
