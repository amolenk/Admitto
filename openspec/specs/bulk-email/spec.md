# bulk-email Specification

## Purpose

The Email module supports sending bulk emails to groups of attendees or external recipients, driven by a persistent `BulkEmailJob` aggregate that tracks lifecycle, recipients, and send progress. Bulk sends fan out over a single SMTP connection per worker pickup and support cooperative cancellation at any point before completion.
