...
public class MessageQueueProcessor(
    ...
    IOptions<MessageQueueProcessorOptions> options,
    ILoggerFactory loggerFactory,
    ILogger<MessageQueueProcessor> logger)
    : BackgroundService
{
    private readonly AzureStorageQueueProcessor _storageQueueProcessor = new(
        queueClient,
    ...
    protected override Task ExecuteAsync(CancellationToken stoppingToken)
    {
        return CreateRetryStrategy("Queue").ExecuteAsync(
            ct => _storageQueueProcessor.ProcessMessagesAsync(HandleMessageAsync, ct),
    ...
    private ResiliencePipeline CreateRetryStrategy(string instanceName)
    {
        ...
        return new ResiliencePipelineBuilder { Name = nameof(MessageQueueProcessor), InstanceName = instanceName }
            .AddRetry(
