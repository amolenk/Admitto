...
public class OpenFgaAuthorizationService(
    OpenFgaClientFactory clientFactory,
    ...
    : IAuthorizationService
{
    ...
    public async ValueTask MigrateAsync()
    {
        ...
        var modelId = await client.TryGetAuthorizationModelIdAsync() ?? await WriteAuthorizationModelAsync(client);

        logger.LogInformation(
            "OpenFGA migration completed. Store ID: {StoreId}, Authorization Model ID: {ModelId}",
            storeId,
            modelId);
    }
    ...
    private async ValueTask<bool> CheckAsync(
        Guid userId,
        string relation,
        string obj,
        CancellationToken cancellationToken = default)
    ...
    private async ValueTask AddTupleAsync(
        string user,
        string relation,
        string obj,
        CancellationToken cancellationToken = default)
    ...
    private async ValueTask<IEnumerable<string>> ListObjectsAsync(
        string user,
        string relation,
        string type,
        CancellationToken cancellationToken = default)
    ...
    private async ValueTask<bool> TupleExistsAsync(
        string user,
        string relation,
        string obj,
        CancellationToken cancellationToken = default)
