...
import { EmailSettingsForm } from "../../settings/email/email-settings-form";
import {
    buildEmailRecipientOptions,
    TestEmailField,
} from "../../settings/email/test-email-settings-button";
    ...
    return (
        <div>
            <EmailSettingsForm
                ...
                title="Sending"
                description="How emails are sent for this event — sender identity and SMTP server."
                callout={(showInheritedCallout || showOverridingCallout) ? (
                    <div className="flex gap-2.5 py-4 text-[13px] text-muted-foreground">
                        <Info className="h-4 w-4 mt-0.5 shrink-0" />
                        <p className="leading-snug">
                            {showInheritedCallout
                                ? "This event uses the team-level email configuration. Save settings here to create an event-specific override. "
