...
async function fetchMembers(teamId: string): Promise<TeamMemberListItemDto[]> {
...
export default function MembersPage() {
    ...
    const { data: members, isLoading } = useQuery({
        ...
        queryFn: () => fetchMembers(params.teamId),
        throwOnError: false,
    });

    const addMember = useMutation({
        mutationFn: async () => {
            await apiClient.post(`/api/teams/${params.teamId}/members`, {
                email: newEmail,
                role: newRole,
            });
        },
    ...
    const changeRole = useMutation({
        mutationFn: async ({ email, newRole }: { email: string; newRole: TeamMembershipRoleDto }) => {
            await apiClient.put(`/api/teams/${params.teamId}/members/${encodeURIComponent(email)}`, {
                newRole,
            });
        },
        onSuccess: () => {
    ...
    const removeMember = useMutation({
        mutationFn: async (email: string) => {
            await apiClient.delete(`/api/teams/${params.teamId}/members/${encodeURIComponent(email)}`);
        },
        onSuccess: () => {
            setError(null);
            queryClient.invalidateQueries({ queryKey: ["team-members", params.teamId] });
