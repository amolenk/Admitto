...
async function fetchApiKeys(teamId: string): Promise<ApiKeyListItemDto[]> {
    return apiClient.get<ApiKeyListItemDto[]>(`/api/teams/${teamId}/api-keys`);
}
...
export default function ApiKeysPage() {
    ...
    const { data: keys, isLoading } = useQuery({
        queryKey: ["api-keys", params.teamId],
        queryFn: () => fetchApiKeys(params.teamId),
        throwOnError: false,
    });

    const createKey = useMutation({
        mutationFn: async () => {
            return apiClient.post<CreateApiKeyHttpResponse>(`/api/teams/${params.teamId}/api-keys`, {
                name: newKeyName,
            });
        },
        onSuccess: (data) => {
            ...
            queryClient.invalidateQueries({ queryKey: ["api-keys", params.teamId] });
        },
        onError: (err: Error) => {
    ...
    const revokeKey = useMutation({
        mutationFn: async (keyId: string) => {
            await apiClient.delete(`/api/teams/${params.teamId}/api-keys/${keyId}`);
        },
        onSuccess: () => {
            setError(null);
            queryClient.invalidateQueries({ queryKey: ["api-keys", params.teamId] });
