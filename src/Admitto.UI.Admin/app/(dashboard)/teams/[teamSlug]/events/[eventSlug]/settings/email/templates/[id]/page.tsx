...
export default function EventEmailTemplateEditorPage({
    ...
    params: Promise<{ teamSlug: string; eventSlug: string; id: string }>;
}) {
    const { teamSlug, eventSlug, id } = use(params);

    const templateApiUrl = `/api/teams/${teamSlug}/events/${eventSlug}/email-templates/${id}`;
    const previewApiUrl = `/api/teams/${teamSlug}/events/${eventSlug}/email-templates/preview`;
    const testSendApiUrl = `/api/teams/${teamSlug}/events/${eventSlug}/email-templates/${id}/test-send`;
    const backHref = `/teams/${teamSlug}/events/${eventSlug}/settings/email/templates`;
    const queryKey = ["event-email-template", teamSlug, eventSlug, id];
