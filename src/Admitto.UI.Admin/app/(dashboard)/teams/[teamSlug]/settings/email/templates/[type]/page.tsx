...
export default function TeamEmailTemplatePage({ params }: { params: Promise<{ teamSlug: string; type: string }> }) {
    const { teamSlug, type } = use(params);

    const queryKey = ["teams", teamSlug, "email-templates", type];
    const { data, isLoading, error } = useQuery({
        queryKey,
        queryFn: async () => {
            try {
                return await apiClient.get<EmailTemplateDto>(`/api/teams/${teamSlug}/email-templates/${type}`);
            } catch (err) {
        ...
        <div>
            <div className="mb-6">
                ...
                <p className="text-[13.5px] text-muted-foreground mt-0.5">
                    Customize the {typeName.toLowerCase()} email template for this team.
                </p>
            </div>
            ...
            {!isLoading && !error && (
                <EmailTemplateForm
                    templateApiUrl={`/api/teams/${teamSlug}/email-templates/${type}`}
                    previewApiUrl={`/api/teams/${teamSlug}/email-templates/${type}/preview`}
                    testSendApiUrl={`/api/teams/${teamSlug}/email-templates/${type}/test-send`}
                    queryKey={queryKey}
                    backHref={backHref}
