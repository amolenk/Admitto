...
namespace Amolenk.Admitto.Core.Registrations.Application.UseCases.TicketedEvents.ArchiveTicketedEvent.AdminApi;
...
public static class ArchiveTicketedEventHttpEndpoint
{
    ...
    private static async ValueTask<NoContent> ArchiveTicketedEvent(
        Guid teamId,
        Guid eventId,
        ICommandHandler<ArchiveTicketedEventCommand> handler,
        [FromKeyedServices(RegistrationsModule.Key)]
        IUnitOfWork unitOfWork,
        CancellationToken cancellationToken)
    {
        var command = new ArchiveTicketedEventCommand(eventId, teamId);
