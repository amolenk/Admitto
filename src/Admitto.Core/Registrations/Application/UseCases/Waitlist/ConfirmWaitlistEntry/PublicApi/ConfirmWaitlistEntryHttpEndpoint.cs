...
public static class ConfirmWaitlistEntryHttpEndpoint
{
    ...
    {
        group
            .MapPost("/waitlist/{ticketTypeId:guid}/confirm", ConfirmWaitlistEntry)
            .WithName(nameof(ConfirmWaitlistEntry));

        return group;
    }

    private static async ValueTask<Ok> ConfirmWaitlistEntry(
        Guid eventId,
        Guid ticketTypeId,
        string token,
