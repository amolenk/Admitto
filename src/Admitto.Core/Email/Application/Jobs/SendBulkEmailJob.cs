...
///   <item>One <see cref="ISmtpBatchSender"/> session per worker pickup serves every recipient.</item>
...
internal sealed class SendBulkEmailJob(
    ...
    ISmtpBatchSender smtpBatchSender,
    ...
    : IJob
{
    ...
    public async Task Execute(IJobExecutionContext context)
    {
        ...
        try
        {
            ...
            {
                // Reconfirmation is reserved for RequestReconfirmationsJob.
                // Legacy rows remain untouched and are never sent here.
                logger.LogWarning(
                    "Ignoring legacy reconfirmation bulk-email job {BulkEmailJobId}.",
                    bulkJobIdValue);
                return;
            }
            ...
            // Phase 2: stream the generic bulk snapshot through a single SMTP session.
            ...
            {
                await using var session = await smtpBatchSender.OpenSessionAsync(settings, ct);
        ...
        CancellationToken ct)
    {
        ...
        try
        {
            ...
            var delivered = await SendWithInlineRetriesAsync(session, message, ct);
