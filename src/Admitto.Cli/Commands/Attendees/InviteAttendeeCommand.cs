...
namespace Amolenk.Admitto.Cli.Commands.Attendees;

public class InviteAttendeeSettings : TeamEventSettings
{
    [CommandOption("--email")]
...
public class InviteAttendeeCommand(IAccessTokenProvider accessTokenProvider, IConfiguration configuration)
    : ApiCommand<InviteAttendeeSettings>(accessTokenProvider, configuration)
{
    public override async Task<int> ExecuteAsync(CommandContext context, InviteAttendeeSettings settings)
    {
        ...
        AnsiConsole.MarkupLine(
            $"[green]✓ Successfully invited attendee '{response.AttendeeId}'.[/]");
        return 0;
    }
}