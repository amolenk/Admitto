...
namespace Amolenk.Admitto.Cli.Commands.Crew;
    ...
    : EventCommandBase<AddSettings>(accessTokenProvider, configuration)
{
    public override async Task<int> ExecuteAsync(CommandContext context, AddSettings settings)
    {
        ...
        var request = new AddCrewAssignmentRequest
        {
        ...
        var response = await CallApiAsync(async client =>
            await client.Teams[teamSlug].Events[eventSlug].CrewAssignments.PostAsync(request));
        if (response is null) return 1;

        AnsiConsole.MarkupLine(
            $"[green]✓ Successfully added crew assignment (ID = '{response.AssignmentId}').[/]");
        return 0;
    }
}