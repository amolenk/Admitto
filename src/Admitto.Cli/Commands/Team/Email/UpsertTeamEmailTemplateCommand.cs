...
public class UpsertTeamEmailTemplateSettings : TeamSettings
{
    [CommandOption("--type")]
...
public class UpsertTeamEmailTemplateCommand(IAdmittoService admittoService, IConfigService configService)
    : AsyncCommand<UpsertTeamEmailTemplateSettings>
{
    public override async Task<int> ExecuteAsync(
        CommandContext context,
        UpsertTeamEmailTemplateSettings settings,
        CancellationToken cancellationToken)
    {
        ...
        var success = await admittoService.SendAsync(
            client => client.UpsertTeamEmailTemplateAsync(teamSlug, null, settings.EmailType!, request, cancellationToken));

        if (!success) return 1;

        AnsiConsoleExt.WriteSuccesMessage($"Successfully updated team-level '{settings.EmailType}' email template.");
        return 0;
    }
}
