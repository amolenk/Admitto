    ...
    : ApiCommand<ShowSettings>(accessTokenProvider, configuration)
{
    public override async Task<int> ExecuteAsync(CommandContext context, ShowSettings settings)
    {
        ...
        var response = await CallApiAsync(async client =>
            await client.Teams[teamSlug].Events[eventSlug].AttendeeRegistrations[settings.RegistrationId!.Value].GetAsync());
        ...
        await using var qrCodeStream = await CallApiAsync(async client =>
            await client.Teams[teamSlug].Events[eventSlug].AttendeeRegistrations[settings.RegistrationId!.Value].QrCode.GetAsync(c =>
            {
