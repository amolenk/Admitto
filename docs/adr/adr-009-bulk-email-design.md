...
## Status
Superseded by [ADR-017](adr-017-reconfirmation-batches-and-requested-deadlines.md) for the generic/admin bulk-email design (D1 and D2) and for reconfirmation batching and scheduling. The event-time-zone decision in D3 remains accepted. The cadence and per-event Quartz-trigger portions of D3 were previously superseded by [ADR-016](adr-016-hourly-reconfirmation-evaluation.md), which ADR-017 now supersedes.

## Context
The Email module historically only sent single transactional emails (one per `AttendeeRegisteredIntegrationEvent`). This ADR records the former design for organizer-initiated **custom bulk** sends and policy-driven **reconfirm** sends, which proposed fan-out to many recipients with strong audit and resume guarantees. That generic bulk capability is not part of the current design; see ADR-017.
...
### D1 — Former design: recipients are snapshotted once when a `BulkEmailJob` starts and frozen for its lifetime
...
### D2 — Former design: bulk fan-out uses a single SMTP connection per worker pickup, with per-recipient `EmailLog` idempotency
...
## Rationale for the historical design
...
### Positive (historical)
- One former bulk-send pipeline would have served both reconfirm and custom-send flows; the Email module had full ownership.
...
### Negative (historical)
- A registration cancelled between snapshot and last-recipient-sent would still receive the email. This was acceptable for the former v1 design.
- The former bulk fan-out path was a second SMTP code path alongside single-send, mitigated by reusing the SMTP sender abstraction in "session" mode.
...
## References
- arc42 chapter 5 — building-block view (Email module and reconfirm sending).
- arc42 chapter 6 — runtime view (§6.9 reconfirm scheduling).
