...
- **Reconfirm sending** — Email owns the projected schedule inputs, minimal `ReconfirmationBatch` lifecycle, and reconfirmation `EmailLog` claims. The single hourly flow, requested-deadline evaluation, batch gates, live eligibility, and shared SMTP session are documented in [§6.9](06-runtime-view.md#69-reconfirm-scheduling-and-cycle-limits-hourly-active-event-evaluation).
