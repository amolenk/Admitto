...
Publishes the `TicketedEventCreated` / `TicketedEventArchived` lifecycle integration events plus `TicketedEventDetailsChanged` and `TicketedEventReconfirmPolicyChanged`, consumed by the Email module to update event context and drive the per-event reconfirm Quartz trigger.
...
- **Branding, reply routing, sender labels, and links** — built-in templates and SMTP delivery read context from Email-owned projections exposed via `IEmailReadStore`: `team_email_context_view` (`TeamEmailContextView`) for team facts such as `Team.AccentColor`, team name, and optional team reply-to email address, and `event_email_context_view` (`EventEmailContextView`) for event facts such as event name, website URL, public slug, time zone, reconfirm policy, self-service ticket count, and lifecycle state. When present, the projected team name is used as the visible MIME `From` display name while the actual `From` address remains the deployment system sender. The team reply-to address is used only for the `Reply-To` header. Organization and Registrations integration events keep these projections eventually consistent.
...
- **Reconfirm sending** — `EvaluateReconfirmJob` is a per-event Quartz trigger derived from Email's projected reconfirm policy and time-zone snapshot; each tick creates a `BulkEmailJob` (`email_type='reconfirm'`) filtered to `Status=Registered AND HasReconfirmed=false`. Candidate selection remains a live Registrations facade query.
