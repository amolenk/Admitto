...
- **Branding and links** — built-in templates read rendering context from Email-owned projections exposed via `IEmailReadStore`: `team_email_context_view` (`TeamEmailContextView`) for team facts such as `Team.AccentColor`, and `event_email_context_view` (`EventEmailContextView`) for event facts such as event name, website URL, public slug, time zone, reconfirm policy, self-service ticket count, and lifecycle state. Organization and Registrations integration events keep these projections eventually consistent.
