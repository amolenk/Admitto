...
Registrations owns the attendance rules and expected-attendee queries: cancelled registrations are excluded, while registered attendees are included even when they have not reconfirmed. Check-in is an admin operation validated by the server against the selected team and event; the scanner supplies the raw `RegistrationId` QR payload and has no offline credential or validation authority. The attendance activity timeline is a timestamp-only Registrations read model fed by the check-in domain event; it is neither a notification nor an audit log.
