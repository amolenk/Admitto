...
Owns the authoritative `TicketedEvent` aggregate (name, dates, IANA `TimeZone`, lifecycle status, and consolidated policy value objects) as well as attendee registration flows (both admin-initiated and public self-service) and ticket type configuration (the `TicketCatalog` aggregate). Ticket types may define an optional maximum number of reconfirmation emails; for a registration, the strictest configured value (the smallest) governs its current reconfirmation cycle. The `Registration` aggregate carries `FirstName`/`LastName`, a lifecycle `Status` (`Registered`/`Cancelled`), a `HasReconfirmed`/`ReconfirmedAt?` pair, and one-way attendance state through nullable `CheckedInAt` — exposed to other modules via `IRegistrationsFacade.QueryRegistrationsAsync`.

Registrations owns the attendance rules and expected-attendee queries: cancelled registrations are excluded, while registered attendees are included even when they have not reconfirmed. Check-in is validated by the server against the selected team and event, either through the signed-in admin scanner or the anonymous shared scanner (§6.6.4); the scanner supplies the raw `RegistrationId` QR payload and has no offline credential or validation authority. The attendance activity timeline is a Registrations read model fed by the check-in domain event, with only an access-context-origin tag as optional metadata (§8, "Attendance check-in"); it is neither a notification nor a general-purpose audit log. `TicketedEvent` also owns a single optional `ScannerLink` (team-administrator-managed, §6.6.4) whose high-entropy secret authorizes the shared scanner to reuse the same check-in path with no Admitto account.
...
Owns all email concerns: server settings, built-in transactional templates, outgoing-email log, and actual SMTP sending.

- **System sender settings** — SMTP host/port, authenticated Admitto-controlled `FromAddress`, deployment `FromDisplayName` (default `Admitto`), and optional credentials are deployment configuration (`Email:System`), not organizer-owned data. The Worker uses these settings for transactional, waitlist, reconfirmation, cancellation, and OTP delivery.
- **Templates** — transactional templates are code-owned built-in embedded resources rendered with Scriban and the team's branding values. They are not persisted and are not organizer-editable. OTP requests use the single `ITransactionalEmailComposer` with a typed verification-code cause intent; the integration-event adapter supplies only typed cause facts to composition, while recipient, idempotency, and other delivery metadata are supplied to `PrepareEmailDelivery`.
...
- **Branding, sender identity, and links** — the single `ITransactionalEmailComposer` reads context from Email-owned projections exposed via `IEmailReadStore`: `team_email_context_view` (`TeamEmailContextView`) for team branding (projected `TeamName` and `Team.AccentColor`) and `event_email_context_view` (`EventEmailContextView`) for event facts such as event name, website URL, public slug, time zone, reconfirm policy, self-service ticket count, and lifecycle state. SMTP transport and sender identity are deployment configuration — `Email:System:FromAddress` plus `Email:System:FromDisplayName`, with no `Reply-To` header — and are never derived from organizer team data because sending on behalf of a team harms deliverability (see ADR-013). Ticket confirmations use the projected `TeamName` as their `team_name` template value; when the team projection is absent, that value defaults to literal `Admitto`. Typed event-scoped composition applies accent and the fixed system font directly to the closed template-value map; `SmtpTransportSettings` contains only deployment-global SMTP connection, authentication, and platform sender settings. Accent color uses the shared `AccentColor` value object (strict `#rrggbb`) owned by `Team` and projected into `team_email_context_view`; font family is deliberately not team-owned or persisted. Unlike `event_email_context_view`, which is fed by several events from two modules and so genuinely holds partial rows, `team_email_context_view` rows are always complete — both source events carry the full team field set. A team whose branding has not reached Email is represented by *no row*, which composition resolves to default branding (`#2563eb`) and default team label `Admitto`. Organization and Registrations integration events keep these projections eventually consistent.
...
- **Reconfirm sending** — Email owns the projected schedule inputs and reconfirmation `EmailLog` claims. The single hourly flow, requested-deadline evaluation, delivery gates, live eligibility, and shared SMTP session are documented in [§6.9](06-runtime-view.md#69-reconfirm-scheduling-and-cycle-limits-hourly-active-event-evaluation).
