...
- **Settings** — a unified `EmailSettings` aggregate scoped by required `TeamId` and nullable `TicketedEventId` (`null` for team-level rows). Settings store SMTP host/port, from-address, auth mode, and (encrypted) credentials. Effective settings for an event are resolved as: event-scoped row → team-scoped row → none.
- **Templates** — an `EmailTemplate` aggregate scoped by required `TeamId` and nullable `TicketedEventId` (`null` for team-level rows), with a unique template name per scope. Template lookup follows the same precedence: event-scoped → team-scoped → built-in default (embedded resource). Rendering uses the Scriban engine.
