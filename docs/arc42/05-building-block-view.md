...
- **Templates** — transactional templates are code-owned built-in embedded resources rendered with Scriban and the team's branding values. They are not persisted and are not organizer-editable. OTP requests use the typed verification-code composer; the integration-event adapter supplies only the plain code and delivery identity while event/team facts come from the Email-owned rendering scope.
