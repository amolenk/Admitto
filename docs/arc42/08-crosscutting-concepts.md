...
The Email module persists one `email.team_email_context_view` row per `team_id` and one `email.event_email_context_view` row per `(team_id, ticketed_event_id)`. These are application read models (`TeamEmailContextView` and `EventEmailContextView` under `Application/Projections/`), maintained by focused role-based projectors (`TeamEmailContextProjector` for Organization team events and `EventEmailContextProjector` for Registrations event events), and exposed only through the module read store (`IEmailReadStore`) — projectors write projections through the read store, mirroring the Registrations `ActivityLogView`/`ActivityLogProjector` convention. Organization publishes team-level created/details-updated events carrying team name and accent color; it never enumerates events for Email branding. SMTP message construction does not consult organizer team identity: the `From` address and display name are deployment configuration and no `Reply-To` is set (see ADR-013). Ticket confirmations use the projected `TeamName` as `team_name`, while an absent team projection supplies the documented literal fallback `Admitto`; the team projection also supplies accent theming. Registrations `TicketedEventDetailsChanged` events update event name, website URL, public slug, and time zone; other Registrations events update reconfirm policy, self-service ticket count, and lifecycle state.

Projection events carry source aggregate versions (`TeamVersion`, `TicketedEventVersion`, and `TicketCatalogVersion`). Projection rows store the last applied source version and ignore older deliveries, so duplicate and late Service Bus messages cannot overwrite newer source state. Projection rows also use EF row-version concurrency; queue handlers save with retryable concurrency semantics so races between workers are retried instead of acknowledged as deterministic failures. The projector may create partial rows so out-of-order delivery can be filled in by later events. Schedule-affecting updates, including policy clearing and archive, change whether an event is enabled and Active for the recurring reconfirm evaluation. The single `ITransactionalEmailComposer` renders closed, explicit mappings for every transactional event after loading one immutable event-scoped composition context. Ticket-confirmation, coupon, cancellation, and verification-code rendering fail/defer deterministically when the event projection is missing or incomplete, while an absent team projection uses the literal team fallback `Admitto` and default accent color. Reconfirmation reuses the same event-scoped context and built-in template for all candidates in one evaluation. The OTP adapter gives the composer only typed verification-code cause facts; it gives recipient identity and `otp-requested:{OtpCodeId}` idempotency metadata to `PrepareEmailDelivery`, and does not pass generic template parameter maps.
...
`Admitto.AppHost/DatabaseScripts/quartz.sql` owns the `QRTZ_` schema initialization for `quartz-db`, using Quartz's PostgreSQL table layout. The schema script is idempotent and is not managed by EF Core migrations because Quartz owns those tables. The clustered persistent store and `[DisallowConcurrentExecution]` on the recurring reconfirmation job ensure that only one Worker executes that job at a time across the cluster. Its legacy durable job and trigger keys remain stable, and Quartz overwrite-existing-data scheduling replaces the persisted definitions when the Worker restarts.
...
`TicketType.ReservedCapacity` (`Domain/Entities/TicketType.cs`) holds back a portion of `MaxCapacity` from self-service claims, for admin/coupon registrations (e.g. an "AI Workshop" ticket type that reserves 20 of 200 slots for direct invitations). Admin and public claims can interleave in any order — admin does not need to wait for public sales to sell out, and vice versa. This is possible because usage is tracked per pool with two counters, not one:

- `TicketType.Claim(ClaimMode mode)` replaces the old enforce/uncapped split. `ClaimMode.Public` is the self-service path: enforced against public availability (throws when sold out or in WaitlistMode). `ClaimMode.Reserved` is the admin/general-coupon path: uncapped, and additionally increments `ReservedUsedCapacity`. `ClaimMode.PublicUncapped` is the waitlist-coupon-redemption path: uncapped like `Reserved`, but counts as public usage (not reserved) because the coupon already redeemed a slot freed from the public pool when it was issued.
- `TicketType.ReservedUsedCapacity` tracks how many `Reserved` claims have been made, independent of `UsedCapacity`. The unconsumed reserved buffer — `HeldBack = max(0, ReservedCapacity - ReservedUsedCapacity)` — is what's withheld from the public pool at any given moment, regardless of *when* reserved claims happened relative to public ones.
- `TicketType.PublicAvailableCapacity(maxCapacity, reservedCapacity)` computes `max(0, maxCapacity - UsedCapacity - HeldBack)` for arbitrary max/reserved values (used to compare before/after availability when either changes). `IsSoldOut` calls it with the current values: sold out when no public slots remain once the unconsumed reserved buffer is set aside. This one definition is the sole gate consulted by `Claim(ClaimMode.Public)`, `GetPublicTicketTypesHandler`'s status mapping, and waitlist activation.
- The reserved buffer is held back from the public pool from the moment `ReservedCapacity` is set — `PublicAvailableCapacity` subtracts the full unconsumed buffer regardless of whether any reserved claims have happened yet. A reserved claim made *before* any public sales therefore does not further shrink `PublicAvailableCapacity` (it decrements `UsedCapacity` and `ReservedUsedCapacity` together, netting no change), which is exactly what makes claim order irrelevant: the public pool is the same size whether the reserved seat was claimed on day one or after the event sells out. Reserved claims beyond the buffer (`ReservedUsedCapacity > ReservedCapacity`) spill into the public pool, same as before. With `ReservedCapacity == 0`, admin claims simply consume public seats — identical to pre-reserved-capacity behavior.
- Release must credit the same counter a claim incremented, so `Registrations.Domain.ValueObjects.TicketTypeSnapshot` (the ticket record stored on `Registration`) carries the `ClaimMode` it was claimed under. `TicketCatalog.Release(IReadOnlyList<TicketTypeSnapshot>)` takes snapshots (not bare IDs) for exactly this reason; `TicketType.ReleaseCapacity(ClaimMode mode = Public)` decrements `ReservedUsedCapacity` only when releasing a `Reserved` claim. Existing persisted registrations with no `mode` in their JSON default to `Public` (the enum's first value), which is the correct interpretation for tickets claimed before this counter existed.
...
- `WaitlistCapacityFreedDomainEvent` is raised against `PublicAvailableCapacity` before/after: raising `MaxCapacity` or lowering `ReservedCapacity` can each free public slots and are both accounted for in the freed-slots calculation in `TicketCatalog.UpdateTicketType`.
- `ReservedCapacity` is a floor for the admin/coupon pool, not a ceiling: `Coupon.Create` (Organiser-sourced) never checks `ReservedCapacity`/`ReservedUsedCapacity`, and `ClaimMode.Reserved` never rejects. An admin can issue and redeem more Organiser coupons than the declared `ReservedCapacity`; once `ReservedUsedCapacity` exceeds `ReservedCapacity`, `HeldBack` clamps to `0` and further reserved claims silently consume public seats (see the `PublicAvailableCapacity` formula above). This is intentional — reserved capacity guarantees admins/coupons a minimum allocation, it does not cap them.
- `ClaimMode.PublicUncapped` (waitlist-coupon redemption) has no `MaxCapacity` guard either, for a different reason: issuing a waitlist coupon does not put any hold on `TicketType`'s counters — the "one coupon per freed slot" invariant lives only in the `Waitlist` aggregate's own bookkeeping (`IssuedCouponCount`). If an admin's uncapped `Reserved` claim consumes the same freed slot before the waitlist attendee redeems, a `MaxCapacity` check on `PublicUncapped` would reject a redemption for a coupon that was already promised to that attendee. The redemption must stay uncapped so the promise made at issuance always holds, even though this means `MaxCapacity` is not a hard ceiling against this path either. This is safe from double-counting because `WaitlistCapacityFreedDomainEvent` and `RegistrationCancelledDomainEvent` are dispatched synchronously in-process inside the same `SaveChangesAsync` call (`DomainEventsInterceptor`) — not via an outbox/message broker — so a freed-slot count cannot be redelivered and counted twice.
